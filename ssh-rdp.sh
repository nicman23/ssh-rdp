...
deps_or_exit(){
  ...
  DEPS_OPT_L=""
  DEPS_R="bash timeout dd ffmpeg grep awk tail xdpyinfo"
...
#Untimed:
VIDEOPLAYER="taskset -c 0 mpv - --input-cursor=no --input-vo-keyboard=no --input-default-bindings=no --hwdec=auto --title="$WTITLE" --untimed --no-cache --profile=low-latency --opengl-glfinish=yes --opengl-swapinterval=0 &> /dev/null"
...
if [ CREATED_AUDIO_CAPTURE_SOURCE ] ; then
